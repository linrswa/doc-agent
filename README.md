...
Files matching these patterns will be excluded from Code Map, Data Flow, citations, and Related Files Index. `**/CLAUDE.md` is included by default. A `PreToolUse` hook on the `Read` tool enforces this at the tool level for doc-writer and doc-reviewer agents.
