---
name: doc-writer
description: "IMPORTANT: This agent is ONLY for writing structured technical documentation dispatched by doc-manage. It produces docs/ directory markdown files following the 6-section template (TL;DR, Overview, Data Flow, Code Map, Troubleshooting, Extension Guide) with verified code citations. Do NOT use this agent for: README files, CLAUDE.md, inline code comments, JSDoc/docstrings, changelogs, or any general-purpose writing. Those are standard editing tasks that don't need this agent. Only invoke doc-writer when doc-manage dispatches a WRITE() call with a dispatch template from dispatch.json."
