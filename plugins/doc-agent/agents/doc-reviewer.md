---
name: doc-reviewer
description: "IMPORTANT: This agent is ONLY for reviewing structured technical documentation produced by doc-writer. It verifies code citations, checks the 6-section structure, validates file paths with Glob, and spot-checks code snippets against actual source. Do NOT use this agent for: reviewing README files, CLAUDE.md, inline comments, JSDoc/docstrings, or general text. Those don't follow the doc-agent format and this reviewer's checklist won't apply. Only invoke doc-reviewer when doc-manage dispatches a REVIEW() call with a review request."
